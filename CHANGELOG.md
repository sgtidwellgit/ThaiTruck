...
- The sdist was pulling in repo-root clutter that isn't needed to install or
  run the package — an untracked local settings file, CI workflow files, and
  the README's image assets — bloating it to 2.4MB for no reason. Switched
  the sdist from an exclude denylist to an explicit include allowlist
  (`src/`, `tests/`, `benchmarks/`, and the top-level docs only), down to 61KB
